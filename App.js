...
const STORAGE_KEY = 'homefit_v9_builder_state';
...
const exerciseLibrary = [
  ...
  { name: 'Dumbbell Lateral Raise', day: 'Push', muscle: 'shoulders', equipment: ['Dumbbells'], sets: 3, reps: '12-15', targetReps: 14, load: 'Light', rest: 75, defaultKg: 7.5 },
  { name: 'Push Ups', day: 'Push', muscle: 'chest', equipment: ['Bodyweight'], sets: 3, reps: 'AMRAP', targetReps: 12, load: 'Light', rest: 90, defaultKg: 0 },
  { name: 'Bodyweight Dips', day: 'Push', muscle: 'arms', equipment: ['Bodyweight'], sets: 3, reps: '8-15', targetReps: 10, load: 'Moderate', rest: 90, defaultKg: 0 },
  ...
  { name: 'Hammer Curl', day: 'Pull', muscle: 'arms', equipment: ['Dumbbells'], sets: 3, reps: '10-12', targetReps: 11, load: 'Light', rest: 90, defaultKg: 10 },
  { name: 'Rear Delt Fly', day: 'Pull', muscle: 'shoulders', equipment: ['Dumbbells'], sets: 3, reps: '12-15', targetReps: 14, load: 'Light', rest: 75, defaultKg: 6 },
  { name: 'Pull Ups', day: 'Pull', muscle: 'back', equipment: ['Bodyweight'], sets: 3, reps: 'AMRAP', targetReps: 8, load: 'Heavy', rest: 150, defaultKg: 0 },
  { name: 'Chin Ups', day: 'Pull', muscle: 'back', equipment: ['Bodyweight'], sets: 3, reps: 'AMRAP', targetReps: 8, load: 'Heavy', rest: 150, defaultKg: 0 },
  ...
  { name: 'Bodyweight Squat', day: 'Legs', muscle: 'legs', equipment: ['Bodyweight'], sets: 3, reps: '15-25', targetReps: 20, load: 'Light', rest: 75, defaultKg: 0 },
  { name: 'Sit Ups', day: 'Legs', muscle: 'core', equipment: ['Bodyweight'], sets: 3, reps: '12-20', targetReps: 15, load: 'Light', rest: 75, defaultKg: 0 },
  { name: 'Crunches', day: 'Legs', muscle: 'core', equipment: ['Bodyweight'], sets: 3, reps: '15-25', targetReps: 20, load: 'Light', rest: 60, defaultKg: 0 },
  { name: 'Plank', day: 'Legs', muscle: 'core', equipment: ['Bodyweight'], sets: 3, reps: '30-60 sec', targetReps: 45, load: 'Light', rest: 60, defaultKg: 0 },
];
...
export default function App() {
  ...
  const [exerciseDayFilter, setExerciseDayFilter] = useState('All');
  ...
  useEffect(() => {
    if (workoutDraft.length === 0) setWorkoutDraft(createWorkoutDraft(generatedWorkout));
  ...
  function addExerciseToWorkout(exercise) {
    const prepared = createWorkoutDraft([{ ...exercise, suggestedKg: suggestWeight(exercise, state.workouts) }])[0];
    setWorkoutDraft(prev => [...prev, prepared]);
  }

  function removeExerciseFromWorkout(index) {
    setWorkoutDraft(prev => prev.filter((_, i) => i !== index));
  }

  function swapExercise(index, replacement) {
    const prepared = createWorkoutDraft([{ ...replacement, suggestedKg: suggestWeight(replacement, state.workouts) }])[0];
    setWorkoutDraft(prev => prev.map((item, i) => i === index ? prepared : item));
  }
  ...
  function clearWorkoutDraft() {
    Alert.alert('Clear workout?', 'This removes the current workout draft.', [
      { text: 'Cancel' },
      { text: 'Clear', onPress: () => setWorkoutDraft([]) },
    ]);
  }
  ...
  function saveWorkoutTemplate() {
    if (!workoutDraft.length) return Alert.alert('No workout', 'Add exercises first.');
    const name = templateName.trim() || `${nextDay} Template ${(state.workoutTemplates || []).length + 1}`;
    const template = {
      id: uid('template'),
      name,
      day: nextDay,
      exercises: workoutDraft.map(ex => ({
        name: ex.name,
        day: ex.day,
        muscle: ex.muscle,
        ...
        load: ex.load,
        rest: ex.rest,
        defaultKg: ex.defaultKg,
      })),
    };
    setState(prev => ({ ...prev, workoutTemplates: [template, ...(prev.workoutTemplates || [])].slice(0, 20) }));
    setTemplateName('');
    Alert.alert('Template saved', `${name} saved.`);
  }

  function loadWorkoutTemplate(template) {
    const prepared = createWorkoutDraft(template.exercises.map(ex => ({ ...ex, suggestedKg: suggestWeight(ex, state.workouts) })));
    setWorkoutDraft(prepared);
  }

  function deleteWorkoutTemplate(id) {
    setState(prev => ({ ...prev, workoutTemplates: (prev.workoutTemplates || []).filter(t => t.id !== id) }));
  }

  function logWorkout() {
    if (!workoutDraft.length) return Alert.alert('No workout available', 'Add exercises first.');
  ...
  function createBackup() {
    ...
    setPatch({ backupText: backup });
    setBackupInput(backup);
  ...
  function restoreBackup() {
    try {
      ...
      setState({ ...defaultState, ...parsed, tab: 'Dashboard' });
      setTargetDraft({ ...defaultState.profile, ...parsed.profile });
      Alert.alert('Restored', 'Backup restored successfully.');
    } catch (error) {
      Alert.alert('Restore failed', 'Backup text was not valid.');
    }
  }
      ...
      <SafeAreaView style={styles.app}>
        ...
        >
          <View style={styles.cameraOverlay}>
            <Text style={styles.hero}>Scan barcode</Text>
            <Text style={styles.text}>Point the camera at the food barcode. Nutrition will load from Open Food Facts.</Text>
            <Button danger label="Cancel scanner" onPress={() => setScannerOpen(false)} />
          </View>
        </CameraView>
      </SafeAreaView>
    );
  ...
  const filteredExercises = allExercises
    .filter(ex => exerciseDayFilter === 'All' || ex.day === exerciseDayFilter)
    .filter(ex => ex.name.toLowerCase().includes(exerciseSearch.toLowerCase()) || ex.muscle.toLowerCase().includes(exerciseSearch.toLowerCase()))
    .filter(ex => ex.equipment.every(eq => enabledEquipment.includes(eq)));
    ...
    <SafeAreaView style={styles.app}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>Smart home gym + nutrition tracker</Text>
        </View>

        <ScrollView contentContainerStyle={styles.body} keyboardShouldPersistTaps="handled">
          {state.tab === 'Dashboard' && (
            <View>
              <Card title="Today" badge={state.profile.goal}>
                <Text style={styles.hero}>{nextDay} Day</Text>
                <Text style={styles.text}>Calories {totals.calories}/{state.profile.calories} kcal</Text>
                <Text style={styles.text}>Protein {totals.protein}/{state.profile.protein}g</Text>
                <View style={styles.progressTrack}>
                  <View style={[styles.progressFill, { width: `${Math.min(100, (totals.calories / Math.max(1, state.profile.calories)) * 100)}%` }]} />
                </View>
              </Card>

              <Card title="Weekly Dashboard" badge="Pro">
                <View style={styles.statGrid}>
                  <StatBox label="Workouts" value={weeklySummary.workouts} />
                  <StatBox label="Avg cals" value={weeklySummary.avgCalories} />
                  <StatBox label="Avg protein" value={`${weeklySummary.avgProtein}g`} />
                  <StatBox label="Weight" value={`${weeklySummary.weightChange}kg`} />
                </View>
                <Text style={styles.muted}>Last 7 days from your logged workouts, foods and weigh-ins.</Text>
              </Card>

              <Card title="Streaks" badge="Motivation">
                <View style={styles.statGrid}>
                  <StatBox label="Training" value={`${streaks.training}d`} />
                  <StatBox label="Nutrition" value={`${streaks.nutrition}d`} />
                  <StatBox label="This week" value={`${weeklySummary.completion}%`} />
                  <StatBox label="Volume" value={`${weeklyVolume}kg`} />
                </View>
              </Card>

              <Card title="Water Tracker" badge={`${waterToday}/${state.waterTarget || 2500}ml`}>
                <View style={styles.progressTrack}>
                  <View style={[styles.progressFill, { width: `${Math.min(100, (waterToday / Math.max(1, state.waterTarget || 2500)) * 100)}%` }]} />
                </View>
                <View style={styles.setRow}>
                  <TextInput style={[styles.tinyInput, { width: 88 }]} value={waterDraft} keyboardType="decimal-pad" onChangeText={setWaterDraft} />
                  <Text style={styles.setUnit}>ml</Text>
                  <Button secondary label="Add water" onPress={() => addWater()} />
                </View>
                <View style={styles.choiceRow}>
                  {[250, 500, 750].map(amount => (
                    <TouchableOpacity key={amount} style={styles.choice} onPress={() => addWater(amount)}>
                      <Text style={styles.choiceText}>+{amount}ml</Text>
                    </TouchableOpacity>
                  ))}
                </View>
                {(state.water || []).filter(w => w.date === today()).slice(-5).reverse().map(w => (
                  <TouchableOpacity key={w.id} onPress={() => removeWater(w.id)}>
                    <Text style={styles.muted}>Logged {w.ml}ml • tap to remove</Text>
                  </TouchableOpacity>
                ))}
              </Card>

              <Card title="Weekly Coaching" badge="AI-style">
                <Text style={styles.text}>{coaching}</Text>
                <Text style={styles.muted}>Uses your logged workouts, meals, weigh-ins and goal. No paid API needed.</Text>
              </Card>

              <Card title="Training Stats">
                <Text style={styles.text}>Workouts logged: {state.workouts.length}</Text>
                <Text style={styles.text}>7-day volume: {weeklyVolume} kg</Text>
                <Text style={styles.text}>Foods logged today: {mealsToday.length}</Text>
                {secondsLeft > 0 && <Text style={styles.hero}>Rest {formatTime(secondsLeft)}</Text>}
              </Card>

              <Card title="Recent PRs">
                {Object.keys(prs).length === 0 && <Text style={styles.muted}>PRs appear after you log workouts.</Text>}
                {Object.entries(prs).slice(0, 5).map(([name, pr]) => <Text key={name} style={styles.text}>{name}: {pr.kg}kg x {pr.reps}</Text>)}
              </Card>
            </View>
          )}
          ...
          {state.tab === 'Train' && (
            <View>
              <Card title={`Workout Builder - ${nextDay}`} badge="Fitbod-style">
                <Text style={styles.muted}>Add, remove, swap, reorder and save your own workout templates.</Text>
                <Text style={styles.text}>Draft volume: {draftVolume}kg</Text>

                <View style={styles.choiceRow}>
                  <Button secondary label="Regenerate" onPress={regenerateWorkout} />
                  <Button danger label="Clear" onPress={clearWorkoutDraft} />
                </View>

                <Input placeholder="Template name e.g. Heavy Push Day" value={templateName} onChangeText={setTemplateName} />
                <Button secondary label="Save current workout as template" onPress={saveWorkoutTemplate} />

                {workoutDraft.length === 0 && <Text style={styles.muted}>No exercises yet. Add exercises from the library below.</Text>}

                {workoutDraft.map((exercise, index) => (
                  <View key={`${exercise.name}-${index}`} style={styles.exerciseRow}>
                    <Text style={styles.exerciseName}>{index + 1}. {exercise.name}</Text>
                    <Text style={styles.muted}>{exercise.sets} sets • {exercise.reps} • {exercise.load} • Rest {exercise.rest}s</Text>
                    <Text style={styles.text}>Suggested: {exercise.suggestedKg}kg</Text>

                    <View style={styles.choiceRow}>
                      <Button secondary label="↑" onPress={() => moveExercise(index, -1)} />
                      <Button secondary label="↓" onPress={() => moveExercise(index, 1)} />
                      <Button danger label="Remove" onPress={() => removeExerciseFromWorkout(index)} />
                    </View>

                    {(exercise.setsDone || []).map((set, setIndex) => (
                      <View key={`${exercise.name}-${setIndex}`} style={styles.setRow}>
                        <Text style={styles.setLabel}>Set {setIndex + 1}</Text>
                        <TextInput style={styles.tinyInput} value={String(set.kg)} keyboardType="decimal-pad" onChangeText={v => updateWorkoutSet(index, setIndex, 'kg', v)} />
                        <Text style={styles.setUnit}>kg</Text>
                        <TextInput style={styles.tinyInput} value={String(set.reps)} keyboardType="decimal-pad" onChangeText={v => updateWorkoutSet(index, setIndex, 'reps', v)} />
                        <Text style={styles.setUnit}>reps</Text>
                      </View>
                    ))}

                    <Button secondary label={`Start ${exercise.rest}s rest`} onPress={() => startRest(exercise.rest)} />
                  </View>
                ))}

                <Button label="Log this workout" onPress={logWorkout} />
              </Card>

              <Card title="Exercise Library" badge="Add / Swap">
                <Input placeholder="Search exercises e.g. row, push up, curl" value={exerciseSearch} onChangeText={setExerciseSearch} />

                <View style={styles.choiceRow}>
                  {['All', 'Push', 'Pull', 'Legs'].map(day => (
                    <TouchableOpacity key={day} style={[styles.choice, exerciseDayFilter === day && styles.choiceOn]} onPress={() => setExerciseDayFilter(day)}>
                ...
                {filteredExercises.map(ex => (
                  <View key={ex.name} style={styles.foodResult}>
                    <Text style={styles.exerciseName}>{ex.name}</Text>
                    <Text style={styles.muted}>{ex.day} • {ex.muscle} • {ex.equipment.join(', ')} • {ex.sets} x {ex.reps}</Text>
                    <Button secondary label="Add to workout" onPress={() => addExerciseToWorkout(ex)} />
                    {workoutDraft.map((w, idx) => (
                      <Button key={`${ex.name}-swap-${idx}`} secondary label={`Swap with ${idx + 1}. ${w.name}`} onPress={() => swapExercise(idx, ex)} />
                    ))}
                  </View>
                ))}
              </Card>

              <Card title="Saved Workout Templates">
                {(state.workoutTemplates || []).length === 0 && <Text style={styles.muted}>No templates saved yet.</Text>}
                {(state.workoutTemplates || []).map(template => (
                  <View key={template.id} style={styles.itemRow}>
                    <TouchableOpacity style={{ flex: 1 }} onPress={() => loadWorkoutTemplate(template)}>
                      <Text style={styles.text}>{template.name}</Text>
                      <Text style={styles.muted}>{template.day} • {template.exercises.length} exercises</Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => deleteWorkoutTemplate(template.id)}>
              ...
              <Card title="Workout History" badge="Tap to view">
                {state.workouts.length === 0 && <Text style={styles.muted}>No workouts logged yet.</Text>}
                {state.workouts.slice(-8).reverse().map(workout => (
                  <TouchableOpacity key={workout.id} style={[styles.historyRow, selectedWorkout?.id === workout.id && styles.historyRowOn]} onPress={() => setSelectedWorkoutId(workout.id)}>
                    <Text style={styles.text}>{workout.date} • {workout.day}</Text>
                    <Text style={styles.muted}>{workout.volume || 0}kg volume • {(workout.exercises || []).length} exercises</Text>
                  </TouchableOpacity>
                ))}
              </Card>

              {selectedWorkout && (
                <Card title={`${selectedWorkout.day} Details`} badge={selectedWorkout.date}>
                  <Text style={styles.heroSmall}>{selectedWorkout.volume || 0}kg</Text>
                  {(selectedWorkout.exercises || []).map(ex => (
                    <View key={ex.name} style={styles.foodResult}>
                      <Text style={styles.exerciseName}>{ex.name}</Text>
                      <Text style={styles.muted}>{(ex.setsDone || []).map((set, idx) => `S${idx + 1}: ${set.kg}kg x ${set.reps}`).join('   ')}</Text>
                    </View>
                  ))}
                </Card>
              )}
          ...
          {state.tab === 'Food' && (
            <View>
              <Card title="Today’s Nutrition">
                <Text style={styles.hero}>{totals.calories} kcal</Text>
                <Text style={styles.text}>Protein {totals.protein}g • Carbs {totals.carbs}g • Fat {totals.fat}g</Text>
              </Card>

              <Card title="Food Search + Barcode">
                <Input placeholder="Search food e.g. chicken breast, Morrisons rice" value={foodQuery} onChangeText={setFoodQuery} />
                <Button label={foodLoading ? 'Searching...' : 'Search foods'} onPress={searchOpenFoodFacts} />
                <Input placeholder="Barcode number" value={barcode} onChangeText={setBarcode} keyboardType="numeric" />
                <Button secondary label="Scan with camera" onPress={openScanner} />
                <Button secondary label="Find barcode by number" onPress={searchBarcode} />
                {foodResults.map(food => <FoodResult key={food.id} food={food} onPress={() => { setSelectedFood(food); setPortionG(String(food.servingG || 100)); }} />)}
              </Card>

              {selectedFood && (
                <Card title="Add Food">
                  <Text style={styles.exerciseName}>{selectedFood.name}</Text>
                  <Text style={styles.muted}>{selectedFood.brand} • per 100g: {selectedFood.calories} kcal</Text>
                  <Input placeholder="Portion grams" value={portionG} onChangeText={setPortionG} keyboardType="decimal-pad" />
                  <Button label="Add to today" onPress={() => addFoodToDiary(selectedFood)} />
                  <Button secondary label="Save as favourite" onPress={() => setPatch({ favourites: [selectedFood, ...state.favourites.filter(f => f.id !== selectedFood.id)] })} />
                </Card>
              )}

              <Card title="Recent / Most Used Foods" badge="Fast log">
                {(state.recentFoods || []).length === 0 && <Text style={styles.muted}>Foods you log will appear here for quicker repeat entries.</Text>}
                {(state.recentFoods || []).slice(0, 8).map(food => <FoodResult key={food.id} food={food} onPress={() => { setSelectedFood(food); setPortionG(String(food.servingG || 100)); }} />)}
              </Card>

              <Card title="Favourites / Everyday Foods">
                {state.favourites.slice(0, 20).map(food => <FoodResult key={food.id} food={food} onPress={() => { setSelectedFood(food); setPortionG(String(food.servingG || 100)); }} />)}
              </Card>

              <Card title="Favourite Meals" badge="One tap">
                <Text style={styles.muted}>Save a normal day meal once, then log it quickly next time.</Text>
                <Input placeholder="Meal name e.g. Work breakfast" value={mealName} onChangeText={setMealName} />
                <Button secondary label="Save today as favourite meal" onPress={saveTodaysMeal} />
                {(state.savedMeals || []).length === 0 && <Text style={styles.muted}>No favourite meals saved yet.</Text>}
                {(state.savedMeals || []).map(meal => (
                  <View key={meal.id} style={styles.itemRow}>
                    <TouchableOpacity style={{ flex: 1 }} onPress={() => addSavedMeal(meal)}>
                      <Text style={styles.text}>{meal.name}</Text>
                      <Text style={styles.muted}>{meal.totals?.calories || 0} kcal • P{meal.totals?.protein || 0} C{meal.totals?.carbs || 0} F{meal.totals?.fat || 0}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => setPatch({ savedMeals: (state.savedMeals || []).filter(item => item.id !== meal.id) })}>
              ...
              <Card title="Today’s Diary">
                {mealsToday.length === 0 && <Text style={styles.muted}>No meals logged today.</Text>}
                {mealsToday.map(meal => (
                  <View key={meal.id} style={styles.itemRow}>
                    <View style={{ flex: 1 }}>
                      <Text style={styles.text}>{meal.name}</Text>
                      <Text style={styles.muted}>{meal.grams}g • {meal.calories} kcal • P{meal.protein} C{meal.carbs} F{meal.fat}</Text>
                    </View>
                    <TouchableOpacity onPress={() => setPatch({ meals: state.meals.filter(item => item.id !== meal.id) })}>
                      <Text style={styles.delete}>Delete</Text>
                    </TouchableOpacity>
                  </View>
                ))}
              ...
              <Card title="Create Custom Food">
                <Input placeholder="Food name" value={customFood.name} onChangeText={v => setCustomFood({ ...customFood, name: v })} />
                <Input placeholder="Serving grams" value={customFood.servingG} onChangeText={v => setCustomFood({ ...customFood, servingG: v })} keyboardType="decimal-pad" />
                {['calories', 'protein', 'carbs', 'fat'].map(key => (
                  <Input key={key} placeholder={`${key} per serving`} value={customFood[key]} onChangeText={v => setCustomFood({ ...customFood, [key]: v })} keyboardType="decimal-pad" />
                ))}
                <Button label="Save custom food" onPress={saveCustomFood} />
              </Card>
            </View>
          )}
          ...
          {state.tab === 'Progress' && (
            <View>
              <Card title="Weight Trend" badge="Graph">
                <WeightGraph progress={state.progress} />
                <Text style={styles.muted}>Uses your last 8 weigh-ins. Keep checking waist as well for recomp.</Text>
              </Card>

              <Card title="Measurements">
                <Text style={styles.muted}>Add weigh-ins here. Auto calorie adjustment uses your recent weight trend.</Text>
                {['weight', 'waist', 'chest', 'arms', 'thighs'].map(key => (
                  <Input key={key} placeholder={`${key} kg/cm`} value={progressDraft[key]} onChangeText={v => setProgressDraft({ ...progressDraft, [key]: v })} keyboardType="decimal-pad" />
                ))}
                <Button label="Save measurements" onPress={() => {
                  setPatch({
                    progress: [...state.progress, {
                      ...progressDraft,
                      id: uid('progress'),
                      date: today(),
                      weight: toNum(progressDraft.weight),
                      waist: toNum(progressDraft.waist),
                      chest: toNum(progressDraft.chest),
                      arms: toNum(progressDraft.arms),
                      thighs: toNum(progressDraft.thighs),
                    }],
                  });
                  setProgressDraft({ weight: '', waist: '', chest: '', arms: '', thighs: '' });
                }} />
                <Button secondary label="Auto-adjust calories from weigh-ins" onPress={autoAdjustCalories} />
              </Card>

              <Card title="History">
                {state.progress.length === 0 && <Text style={styles.muted}>No measurements yet.</Text>}
                {state.progress.slice(-10).reverse().map(row => <Text key={row.id} style={styles.text}>{row.date} • {row.weight}kg • waist {row.waist}cm</Text>)}
              </Card>

              <Card title="PR Board">
                {Object.keys(prs).length === 0 && <Text style={styles.muted}>No PRs yet.</Text>}
                {Object.entries(prs).map(([name, pr]) => <Text key={name} style={styles.text}>{name}: {pr.kg}kg x {pr.reps}</Text>)}
              </Card>
            </View>
          )}
          ...
          {state.tab === 'More' && (
            <View>
              <Card title="Goals">
                {goals.map(goal => (
                  <TouchableOpacity key={goal.key} style={[styles.goal, targetDraft.goal === goal.key && styles.goalOn]} onPress={() => calculateTargets(goal.key, toNum(targetDraft.weight))}>
                    <Text style={styles.goalTitle}>{goal.emoji} {goal.key}</Text>
                    <Text style={styles.muted}>{goal.note}</Text>
                  </TouchableOpacity>
                ))}
              </Card>

              <Card title="Targets">
                <LabelInput label="Daily Calories" value={String(targetDraft.calories)} onChangeText={v => setTargetDraft({ ...targetDraft, calories: v })} />
                <LabelInput label="Protein (g)" value={String(targetDraft.protein)} onChangeText={v => setTargetDraft({ ...targetDraft, protein: v })} />
                <LabelInput label="Carbs (g)" value={String(targetDraft.carbs)} onChangeText={v => setTargetDraft({ ...targetDraft, carbs: v })} />
                <LabelInput label="Fat (g)" value={String(targetDraft.fat)} onChangeText={v => setTargetDraft({ ...targetDraft, fat: v })} />
                <LabelInput label="Body Weight (kg)" value={String(targetDraft.weight)} onChangeText={v => setTargetDraft({ ...targetDraft, weight: v })} />
                <LabelInput label="Water Target (ml)" value={String(state.waterTarget || 2500)} onChangeText={v => setPatch({ waterTarget: toNum(v) || 2500 })} />
                <Button label="Save Targets" onPress={saveTargets} />
                <Button secondary label="Calculate Targets" onPress={() => calculateTargets(targetDraft.goal, toNum(targetDraft.weight))} />
                <Button secondary label="Reset Recommended" onPress={() => { const reset = defaultState.profile; setTargetDraft(reset); setState(prev => ({ ...prev, profile: reset })); }} />
              </Card>

              <Card title="Equipment Manager">
                {state.equipment.map((item, index) => (
                  <View key={item.id} style={styles.itemRow}>
                    <TouchableOpacity style={{ flex: 1 }} onPress={() => {
                      const equipment = [...state.equipment];
                      equipment[index] = { ...equipment[index], enabled: !equipment[index].enabled };
                      setPatch({ equipment });
                    }}>
                      <Text style={styles.text}>{item.enabled ? '✓' : '☐'} {item.name}</Text>
                      <Text style={styles.muted}>Max {item.maxKg || '-'} kg</Text>
                    </TouchableOpacity>
                    <TextInput style={styles.smallInput} value={String(item.maxKg)} keyboardType="decimal-pad" onChangeText={v => {
                      const equipment = [...state.equipment];
                      equipment[index] = { ...equipment[index], maxKg: toNum(v) };
                      setPatch({ equipment });
                    }} />
                    <TouchableOpacity onPress={() => setPatch({ equipment: state.equipment.filter(eq => eq.id !== item.id) })}>
                      <Text style={styles.delete}>Remove</Text>
                    </TouchableOpacity>
                  </View>
                ))}
                <Input placeholder="New equipment name" value={equipmentDraft.name} onChangeText={v => setEquipmentDraft({ ...equipmentDraft, name: v })} />
                <Input placeholder="Max kg e.g. 17.5" value={equipmentDraft.maxKg} onChangeText={v => setEquipmentDraft({ ...equipmentDraft, maxKg: v })} keyboardType="decimal-pad" />
                <Button label="Add equipment" onPress={() => {
                  if (!equipmentDraft.name.trim()) return;
                  setPatch({ equipment: [...state.equipment, { id: uid('eq'), name: equipmentDraft.name.trim(), enabled: true, maxKg: toNum(equipmentDraft.maxKg) }] });
                  setEquipmentDraft({ name: '', maxKg: '' });
                }} />
              </Card>

              <Card title="Manual Exercise Creator" badge="Custom">
                <Text style={styles.muted}>Add your own lifts, bodyweight moves or cable/bar variations. Use equipment names that match your equipment list.</Text>
                <Input placeholder="Exercise name e.g. Incline Dumbbell Curl" value={exerciseDraft.name} onChangeText={v => updateExerciseDraft('name', v)} />
                <View style={styles.choiceRow}>
                  {['Push', 'Pull', 'Legs'].map(day => (
                    <TouchableOpacity key={day} style={[styles.choice, exerciseDraft.day === day && styles.choiceOn]} onPress={() => updateExerciseDraft('day', day)}>
                      <Text style={styles.choiceText}>{day}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
                <Input placeholder="Muscle e.g. arms" value={exerciseDraft.muscle} onChangeText={v => updateExerciseDraft('muscle', v)} />
                <Input placeholder="Equipment e.g. Dumbbells or Bodyweight" value={exerciseDraft.equipment} onChangeText={v => updateExerciseDraft('equipment', v)} />
                <View style={styles.grid2}>
                  <Input placeholder="Sets" value={exerciseDraft.sets} onChangeText={v => updateExerciseDraft('sets', v)} keyboardType="decimal-pad" />
                  <Input placeholder="Reps e.g. 8-12" value={exerciseDraft.reps} onChangeText={v => updateExerciseDraft('reps', v)} />
                </View>
                <View style={styles.grid2}>
                  <Input placeholder="Target reps" value={exerciseDraft.targetReps} onChangeText={v => updateExerciseDraft('targetReps', v)} keyboardType="decimal-pad" />
                  <Input placeholder="Start kg" value={exerciseDraft.defaultKg} onChangeText={v => updateExerciseDraft('defaultKg', v)} keyboardType="decimal-pad" />
                </View>
                <View style={styles.choiceRow}>
                  {['Heavy', 'Moderate', 'Light'].map(load => (
                    <TouchableOpacity key={load} style={[styles.choice, exerciseDraft.load === load && styles.choiceOn]} onPress={() => updateExerciseDraft('load', load)}>
                      <Text style={styles.choiceText}>{load}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
                <Input placeholder="Rest seconds e.g. 90" value={exerciseDraft.rest} onChangeText={v => updateExerciseDraft('rest', v)} keyboardType="decimal-pad" />
                <Button label="Add custom exercise" onPress={saveCustomExercise} />

                {(state.customExercises || []).map(ex => (
                  <View key={ex.name} style={styles.itemRow}>
                    <View style={{ flex: 1 }}>
                      <Text style={styles.text}>{ex.name}</Text>
                      <Text style={styles.muted}>{ex.day} • {ex.equipment.join(', ')} • {ex.sets} x {ex.reps}</Text>
                    </View>
                    <TouchableOpacity onPress={() => setPatch({ customExercises: (state.customExercises || []).filter(item => item.name !== ex.name) })}>
                      <Text style={styles.delete}>Remove</Text>
                    </TouchableOpacity>
                  </View>
                ))}
              ...
              <Card title="Backup & Restore">
                <Text style={styles.muted}>One backup includes workouts, nutrition, progress, equipment, settings and goals.</Text>
                <Button label="Create Backup" onPress={createBackup} />
                <Input multiline placeholder="Backup text appears here. Paste backup text here to restore." value={backupInput} onChangeText={setBackupInput} />
                <Button secondary label="Restore Backup" onPress={restoreBackup} />
                <Button danger label="Reset App" onPress={() => Alert.alert('Reset app?', 'This deletes local app data.', [
                  { text: 'Cancel' },
                  { text: 'Reset', onPress: () => { setState(defaultState); setTargetDraft(defaultState.profile); } },
                ])} />
              </Card>
            </View>
          )}
        ...
        <View style={styles.bottomTabs}>
          {['Dashboard', 'Train', 'Food', 'Progress', 'More'].map(tab => (
            <TouchableOpacity key={tab} style={styles.bottomTab} onPress={() => setPatch({ tab })}>
              <Text style={[styles.bottomIcon, state.tab === tab && styles.bottomOn]}>{tabIcon(tab)}</Text>
              <Text style={[styles.bottomLabel, state.tab === tab && styles.bottomOn]}>{tab}</Text>
            </TouchableOpacity>
          ))}
        </View>
      </KeyboardAvoidingView>
...
function workoutVolume(exercises) {
  return round1(exercises.reduce((sum, ex) => sum + (ex.setsDone || []).reduce((s, set) => s + toNum(set.kg) * toNum(set.reps), 0), 0));
}

function volumeLast7Days(workouts) {
  const cutoff = new Date();
  cutoff.setDate(cutoff.getDate() - 7);
  return round1(workouts.filter(w => new Date(w.date) >= cutoff).reduce((sum, w) => sum + toNum(w.volume), 0));
}

function getPRs(workouts) {
  const prs = {};
  workouts.forEach(w => (w.exercises || []).forEach(ex => (ex.setsDone || []).forEach(set => {
    const kg = toNum(set.kg);
    const reps = toNum(set.reps);
    const score = kg * reps;
    if (!prs[ex.name] || score > prs[ex.name].score) prs[ex.name] = { kg, reps, score };
  })));
  return prs;
}
...
function mapOpenFoodFacts(product) {
  ...
  const name = product.product_name || 'Unnamed food';
  ...
  return {
    ...
    servingG: toNum(product.serving_quantity) || 100,
    calories: round1(toNum(calories)),
...
function macroTotals(items) {
  return items.reduce((acc, item) => ({
    calories: round1(acc.calories + toNum(item.calories)),
    protein: round1(acc.protein + toNum(item.protein)),
    carbs: round1(acc.carbs + toNum(item.carbs)),
    fat: round1(acc.fat + toNum(item.fat)),
...
function tabIcon(tab) {
  return { Dashboard: '🏠', Train: '🏋️', Food: '🍽️', Progress: '📈', More: '⚙️' }[tab];
}

function Card({ title, badge, children }) {
  return <View style={styles.card}><View style={styles.cardHeader}><Text style={styles.cardTitle}>{title}</Text>{badge && <Text style={styles.badge}>{badge}</Text>}</View>{children}</View>;
}
...
function Button({ label, onPress, secondary, danger }) {
  return <TouchableOpacity onPress={onPress} style={[styles.button, secondary && styles.secondaryButton, danger && styles.dangerButton]}><Text style={styles.buttonText}>{label}</Text></TouchableOpacity>;
}

function Input(props) {
  return <TextInput {...props} placeholderTextColor="#7f8798" style={[styles.input, props.multiline && styles.multiline]} />;
}

function LabelInput({ label, value, onChangeText }) {
  return <View style={{ marginBottom: 10 }}><Text style={styles.label}>{label}</Text><Input value={value} onChangeText={onChangeText} keyboardType="decimal-pad" /></View>;
}

function FoodResult({ food, onPress }) {
  return <TouchableOpacity style={styles.foodResult} onPress={onPress}><Text style={styles.text}>{food.name}</Text><Text style={styles.muted}>{food.brand} • {food.calories} kcal/100g • P{food.protein} C{food.carbs} F{food.fat}</Text></TouchableOpacity>;
}
...
function WeightGraph({ progress }) {
  ...
  return <View><View style={styles.graph}>{rows.map(row => {
    const weight = toNum(row.weight);
    const height = 28 + ((weight - min) / range) * 78;
    return <View key={row.id} style={styles.graphCol}><View style={[styles.graphBar, { height }]} /><Text style={styles.graphLabel}>{weight}</Text></View>;
  })}</View><Text style={styles.text}>Latest: {weights[weights.length - 1]}kg • Change: {round1(weights[weights.length - 1] - weights[0])}kg</Text></View>;
}
...
const styles = {
  ...
  input: { flex: 1, backgroundColor: '#0a0f1d', borderColor: '#313b58', borderWidth: 1, color: '#fff', borderRadius: 15, paddingHorizontal: 13, paddingVertical: 12, marginVertical: 5, fontSize: 16 },
  multiline: { minHeight: 90, textAlignVertical: 'top' },
  button: { backgroundColor: '#4d6dff', paddingVertical: 14, paddingHorizontal: 14, borderRadius: 17, alignItems: 'center', marginTop: 10, borderWidth: 1, borderColor: '#6f86ff' },
  ...
  setRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 8, flexWrap: 'wrap' },
